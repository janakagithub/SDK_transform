package SDK_transform::SDK_transformServer;
...
our %return_counts = (
        'transform_script' => 1,
        'transform_script_async' => 1,
        'transform_script_check' => 1,
        'version' => 1,
);

our %method_authentication = (
        'transform_script' => 'required',
        'transform_script_async' => 'required',
        'transform_script_check' => 'required',
);
...
our %async_run_methods = (
        'transform_script_async' => 'SDK_transform.transform_script',
);

our %async_check_methods = (
        'transform_script_check' => 'SDK_transform.transform_script',
);
...
sub call_method {
    ...
    my $ctx = SDK_transform::SDK_transformServerContext->new($self->{loggers}->{userlog},
...
{
    # Service SDK_transform requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for SDK_transform but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = SDK_transform::SDK_transformServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service SDK_transform.");
    }
...
package SDK_transform::SDK_transformServerContext;

use strict;

=head1 NAME

SDK_transform::SDK_transformServerContext
...
package SDK_transform::SDK_transformServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = SDK_transform::SDK_transformServer->new(
